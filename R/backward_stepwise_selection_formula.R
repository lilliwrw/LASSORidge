#' Backward stepwise selection of coefficients (using formula argument)
#'
#'
#' \code{backward_stepwise_selection_formula} is used to calculate, which coefficients in
...
backward_stepwise_selection_formula <- function(
  ...
  # Check variable formula for malformed input
  ...
  if(is.null(nparam)) nparam <- 1:length(variables)
  ...
  stopifnot("'nparam' contains negative subset size" = nparam >-1L,
            "'nparam' contains subset size greater than the number of coefficients" = nparam <= length(variables)
  )
  ...
  res <- as.list(rep(NA, times = length(variables)))
  names(res) <- as.character(1:length(variables))
  ...
  if (min_params<length(undropped_params)) {
    for (i in (length(undropped_params)-1):min_params) {
      ...
      # Add best parameter to result and prepare formula for the next iteration
      formula <- remove_variable(formula, undropped_params[next_param])
